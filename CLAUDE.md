...
| 코드 실행 | Judge0 via RapidAPI (무료 티어) |
| 배포 | Vercel |
| 인증 | NextAuth.js |
| PWA | next-pwa |
...
- **코딩 문제**: 빈칸 채우기 형식 (코드 일부를 `___`로 비워둠) + Judge0로 실제 실행
...
Body:
  {
    "source_code": "완성된 코드",
    "language_id": 71,  // Python: 71, JS: 63, Java: 62
    "stdin": ""
  }
```

- 무료 티어 한도 내에서 운영 (콘테스트 기간 충분)
...
**분석 로직:**
```
user_answers 테이블에서 최근 7일(또는 30일) 오답 기록 조회
    ↓
OpenAI에게 패턴 분석 요청
    ↓
"반복문 관련 문제에서 자주 틀림", "함수 개념이 취약함" 등 요약
    ↓
앱 내 리포트 페이지 + 푸시 알림으로 전달
```
...
create table questions (
  ...
  code_template text,      -- 코딩 문제 빈칸 템플릿
  expected_output text,    -- 코딩 문제 예상 출력값
...
create table user_answers (
  ...
  subject_id uuid references subjects(id),
  attempt int check (attempt in (1, 2)) default 1,  -- 1차/2차 시도
  answer text,
  is_correct boolean,
